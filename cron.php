...
if (!$utility->is_cli()) {
    ...
    if (!$appInfo->send_cron_notification) {
        die();
    } else {
        $readyForCron = $utility->db->getAllRecords("transactions", "*", "AND status = 1");
    
        foreach ($readyForCron as $txn) {
            $event = $api->verifyOrder($txn->order_id);
